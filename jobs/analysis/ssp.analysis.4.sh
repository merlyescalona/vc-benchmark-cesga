  ...
  for sizeID in ${sizes[*]}; do
    for replicateST in ${replicates[*]}; do
      ...
      for indID in $(seq 0 $nInds); do
        ...
        echo "samtools view -bSh $outfile | samtools sort - -o $outputDIR/$outputFILE -@ 4" >> "${script}/${pipelinesName}.${replicateID}.${profileFOLDER}.samtools.sh"
...
split -l 300 -d -a 4 "$HOME/data/mappings/${pipelinesName}.${replicateID}/scripts/${pipelinesName}.${replicateID}.${profileFOLDER}.sh" "$HOME/data/mappings/${pipelinesName}.${replicateID}/scripts/${pipelinesName}.${replicateID}.${profileFOLDER}.samtools.commands."
