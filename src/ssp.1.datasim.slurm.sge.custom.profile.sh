...
for item in $(cat $indelibleFolders);do
    cd $item
    echo -e "$item; gt $(ls g_trees* | wc -l)\tFASTA $(ls *.fasta | grep TRUE | wc -l)\tTRUEFASTA $(ls *TRUE.fasta | wc -l)"
done
################################################################################
# 4. ngsphy
################################################################################
...
# @ triploid
################################################################################
step3.1JOBID=$(qsub -t $simphyReplicateID  $HOME/src/vc-benchmark-cesga/jobs/vcs.3.1.references.sh | awk '{ print $4}')
################################################################################
# 4. 0
#-------------------------------------------------------------------------------
